				...
				if (!empty($aMessage['ERRORS'])) {
					foreach($aMessage['ERRORS'] as $aError) {
						if ($aError['statusCode'] == 0) {
							//Logger::LogFile("INFO: Message ID {$k} {$sCustomIdentifier} has no error ({$aError['statusCode']}), removing from queue...");
							$this->_removeMessageFromQueue($k);
							continue 2;
						} else if ($aError['statusCode'] > 1 && $aError['statusCode'] <= 8) {
							//Logger::LogFile("WARNING: Message ID {$k} {$sCustomIdentifier} has an unrecoverable error ({$aError['statusCode']}), removing from queue without retrying...");
...
//						);
				...
				//Logger::LogFile("STATUS: Sending message ID {$k} {$sCustomIdentifier} (" . ($nErrors + 1) . "/{$this->_nSendRetryTimes}): {$nLen} bytes.");
			...
			if (!$bError) {
				...
				if ($nChangedStreams === false) {
					//Logger::LogFile('ERROR: Unable to wait for a stream availability.');
					break;
...
//		Logger::LogFile('ERROR: Unable to send message ID ' .
